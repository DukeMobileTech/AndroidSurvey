...
public abstract class ListOfItemsFragment extends RosterFragment {
    ...
    protected void createResponseComponent(ViewGroup responseComponent) {
        ...
        for (int i = 0; i < getQuestion().defaultOptions().size(); i++) {
            ...
            responseComponent.addView(optionText);
            EditText editText = createEditText();
            editText.setHint(R.string.free_response_edittext);
            editText.setTypeface(getQuestion().getInstrument().getTypeFace(getActivity().getApplicationContext()));

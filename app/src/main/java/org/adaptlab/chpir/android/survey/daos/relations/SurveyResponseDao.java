...
@Dao
public abstract class SurveyResponseDao extends BaseDao<Survey> {
    @Transaction @Query("SELECT * FROM Surveys WHERE UUID=:uuid LIMIT 1")
    public abstract LiveData<SurveyResponse> findByUUID(String uuid);

    @Transaction @Query("SELECT * FROM Surveys WHERE ProjectId=:projectId")
